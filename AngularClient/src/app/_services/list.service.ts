...
export class ListService {
  ...
  private refreshLists() {
    this.httpClient.get<ListUser[]>(`${this.url}/users`).subscribe((lists) => {
  ...
  createList(list: ListUser): Observable<string> {
    return this.httpClient.post(`${this.url}/users`, list, {
      responseType: 'text',
    });
  }
  ...
  updateList(id: number, list: object) {
    return this.httpClient.put(`${this.url}/users/${id}`, list, {
      responseType: 'text',
    });
  }
